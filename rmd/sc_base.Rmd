...
```{r Query, child=system.file('rmd', 'sc_step7.Rmd', package = 'HemaScopeR'), eval=Step7_Query_Data}
```

```{r CellCycle, child=system.file('rmd', 'sc_step8.Rmd', package = 'HemaScopeR'), eval=Step8_Assign_Cell_Cycle}
```

```{r Heterogeneity, child=system.file('rmd', 'sc_step9.Rmd', package = 'HemaScopeR'), eval=Step9_Calculate_Heterogeneity}
```

```{r MarkerGenes, child=system.file('rmd', 'sc_step10.Rmd', package = 'HemaScopeR'), eval=Step10_Violin_Plot_fo_Marker_Genes}
```

```{r Lineage, child=system.file('rmd', 'sc_step11.Rmd', package = 'HemaScopeR'), eval=Step11_Calculate_Lineage_Scores}
```
...
```{r GSVA, child=system.file('rmd', 'sc_step14.Rmd', package = 'HemaScopeR'), eval=Step14_GSVA}
```

```{r Interaction, child=system.file('rmd', 'sc_step15.Rmd', package = 'HemaScopeR'), eval=Step15_Cell_Cell_Interaction}
```
