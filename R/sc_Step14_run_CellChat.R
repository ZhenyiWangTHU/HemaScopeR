...
run_CellChat = function(data.input=NULL,
  ...
  samples<-sc_object@meta.data$orig.ident
  identity <- data.frame(labels = factor(labels, levels = cell.orders),samples = samples,row.names = sample.names)
  ...
  if(Org =='mmu'){
    ...
    cellchat <- smoothData(cellchat, adj = PPI.human) #zyt add this code
