...
#'
#' @param st_obj The SeuratObject of merfish data
...
#'
#' @details
#' This function performs basic quality control on the SeuratObject of merfish data and
...
QC_MERFISH <- function(
    ...
    ##### Plot nUMI #####
    p.nUMI.spatial <- myImageFeaturePlot(st_obj = st_obj,
    ...
    ##### Plot nGene #####
    p.nGene.spatial <- myImageFeaturePlot(st_obj = st_obj,
    ...
    st_obj$Quality <- ifelse(st_obj[[paste0('nCount_', active.assay)]] > max.nUMI | 
                                 st_obj[[paste0('nCount_', active.assay)]] < min.nUMI |
                                 st_obj[[paste0('nFeature_', active.assay)]] > max.gene | 
                                 st_obj[[paste0('nFeature_', active.assay)]] < min.gene,
                             'Low',
                             'High')
    suppressMessages(suppressWarnings(
        p.quality.spatial <- ImageDimPlot(st_obj, group.by = 'Quality') +
    ...
    nSpot <- data.frame(nSpot = rowSums(as.matrix(GetAssayData(st_obj, slot = 'count') > 0)))
    ...
    if(species %in% c('human', 'mouse')){
        ...
        p.mito.spatial <- myImageFeaturePlot(st_obj = st_obj,
                                             features = 'percent.mt',
