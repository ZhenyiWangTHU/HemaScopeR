#' The pipeline for analyzing MERFISH data.
#'
#' Function \code{st_MERFISH_pipeline} encompasses the complete downstream
...
#' @param tech Technique used for MERFISH data. Now 'Vizgen', 'Xenium', 'Nonastring' and 'Akoya' are supported.
...
st_MERFISH_pipeline <- function(
    ...
    if(Step5_SVFs){
        ...
        st_obj <- MERFISH_SpatiallyVariableFeatures(
            st_obj = st_obj,
    ...
    if(Step6_Interaction){
        ...
        MERFISH_Interaction(
            ...
            coordinates_index_col = coordinates_index_col,
